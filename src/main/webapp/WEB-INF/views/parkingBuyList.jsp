			...
			<div id="navbarCollapse" class="collapse navbar-collapse">
				<ul class="nav navbar-nav">
					<c:forEach items="${parkingList}" var="dictParking">
						<c:choose>
							<c:when
								test="${dictParking.parkingId == selectParking.parkingId}">
								<li class="active"><a
									href="${pageContext.request.contextPath}/wxredirect/parkingBuyList?selectParking=${dictParking.parkingId}">${dictParking.parkingName}</a></li>
							</c:when>
							<c:otherwise>
								<li><a
									href="${pageContext.request.contextPath}/wxredirect/parkingBuyList?selectParking=${dictParking.parkingId}">${dictParking.parkingName}</a></li>
		...
		<table class="table table-hover">
			...
			<tbody>
				<c:forEach items="${offerList}" var="parkingOffer">
					<tr class="info">
						<td><fmt:formatDate value='${parkingOffer.validStartTime}'
								type='date' pattern='HH:mm:SS' /> 至 <fmt:formatDate
								value='${parkingOffer.validEndTime}' type='date'
								pattern='HH:mm:SS' /></td>
						<td>${parkingOffer.wxNickName}</td>
						<td>${parkingOffer.price}(元)</td>
						<td>
							<button type="button" class="btn btn-primary" data-toggle="modal"
								data-target="#offerModal" data-offerid="${parkingOffer.offerId}"
								data-stime="${parkingOffer.startTime}"
								data-etime="${parkingOffer.endTime}"
								data-price="${parkingOffer.price}">购买</button>
						</td>
					</tr>
				</c:forEach>
			</tbody>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form:form id="prepayform" class="form-horizontal" modelAttribute="parkingOrderModel"
							action="/wxPayParking/getJSSDKPayInfo" method="post">
							<div class="form-group">
								<label class="control-label col-xs-6" id="pricedesc" for="recipient-name"></label>
								<input type="text" hidden="true" id="selectOfferId"
									name="selectOfferId"> 
							...
							<div class="form-group">
								<div class="col-xs-12">
									<textarea id="detail" name="detail" type="text"
										class="form-control" placeholder="可选择留下您的联系方式，便于联络买家"></textarea>
								</div>
							</div>
							<div class="form-group">
