...
CREATE TABLE `parking_order` (
  ...
  `wx_from_order_id` varchar(64) NOT NULL DEFAULT '-1' COMMENT '微信订单ID,收取订单',
  `wx_to_order_id` varchar(64) NOT NULL DEFAULT '-1' COMMENT '微信订单ID,支付订单',
...
CREATE TABLE `coupon_offer` (
  `coupon_id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '券ID',
  `coupon_brand` varchar(64) NOT NULL COMMENT '所属商家：startbuck;',
  `coupon_type` int(11) unsigned NOT NULL COMMENT '券类型：1-startbuck-regular;2-startbuck-large',
  `offer_status` int(11) NOT NULL DEFAULT '1' COMMENT '券状态：1-发布中；2-已售出；3-暂时失效（被预定）；4-没用了;',
  `price` decimal(8,2) NOT NULL COMMENT '售价',
  `coupon_qrcode` varchar(64) NOT NULL COMMENT '优化券二维码',
  `detail` varchar(64) NOT NULL DEFAULT '' COMMENT '优化券描述',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `dml_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据更新日期',
  PRIMARY KEY (`coupon_id`),
  KEY `coupon_brand_key` (`coupon_brand`),
  KEY `coupon_type_key` (`coupon_type`)
...
CREATE TABLE `coupon_order` (
  `coupon_order_id` char(32) NOT NULL,
  ...
  `wx_openid_buier` varchar(64) NOT NULL COMMENT '买家微信ID',
  `wx_nick_name_buier` varchar(64) NOT NULL COMMENT '买家微信匿名',
  `order_status` int(11) NOT NULL DEFAULT '1' COMMENT '订单户状态：0-订单已提交；3-交易成功；4-交易失败',
  `wx_order_id` varchar(64) NOT NULL DEFAULT '-1' COMMENT '微信订单ID',
  `paid_amount` decimal(8,2) NOT NULL COMMENT '支付价格',
  `latitude` float(10,6) NOT NULL DEFAULT '0.000000' COMMENT '纬度',
  `longitude` float(10,6) NOT NULL DEFAULT '0.000000' COMMENT '经度 ',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  ...
  KEY `user_id_buier_key` (`wx_openid_buier`),
  KEY `coupon_id_key` (`coupon_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='优惠券订单表';
