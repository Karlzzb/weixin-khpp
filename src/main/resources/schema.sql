CREATE TABLE `user` (
  ...
  `user_state` tinyint(4) NOT NULL DEFAULT '1' COMMENT '用户状态：1-关注；2-取消关注',
...
CREATE TABLE `parking_order` (
  ...
  `order_status` tinyint(4) NOT NULL DEFAULT '1' COMMENT '订单户状态：1-买家已支付；2-卖家发货；3-买家确认（交易成功）；4-交易失败',
  ...
  `location` GEOMETRY NOT NULL COMMENT '订单创建位置',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `dml_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据更新日期',
...
CREATE TABLE `parking_offer` (
  ...
  `parking_name` varchar(64) NOT NULL COMMENT '停车场名称',
  `user_id_sellor` int(11) unsigned NOT NULL COMMENT '卖家ID',
  `offer_status` tinyint(4) NOT NULL DEFAULT '1' COMMENT '卡券状态：1-发布中；2-已售出；3-失效;',
  `price` DECIMAL(8,2) NOT NULL COMMENT '卖家售价',
  `location` GEOMETRY NOT NULL COMMENT '卡券发布位置',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `dml_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据更新日期',
